...
{
    FILE* file = fopen("get_time_once.wasm", "rb");
    long size;
    if (!file)
        return 0;
...
int main(void)
{
    ...
    OsStatus status;
    g_log = fopen("get_time_once.log", "w");
    if (!g_log)
        return 1;
    hal_init();
    expect(os_init() == OS_STATUS_OK, "initialize OS");
    os_tick(1234U);
    expect(os_get_tick_ms() == 1234U, "host tick advanced to 1234 ms");
    expect(load_wasm(&wasm), "load get_time_once.wasm");
    if (wasm.bytes)
    {
        status = os_task_create(&task, wasm.bytes, wasm.size, "app_main", "get_time_once",
                                64U * 1024U, OS_TASK_PRIORITY_NORMAL);
        expect(status == OS_STATUS_OK && task != NULL, "create get-time task");
        if (task)
        {
            expect(schedule_until_dead(task), "task reaches DEAD");
            expect(os_task_get_exit_reason(task) == OS_TASK_EXIT_RETURNED,
                   "task returned normally");
            expect(os_task_get_exit_code(task) == 0U, "guest observed 1234 ms");
