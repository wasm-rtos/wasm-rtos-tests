...
int main(void) { WasmBinary wasm = { 0 }; OsTaskHandle task = NULL; OsStatus status; g_log = fopen("yield_once.log", "w"); if (!g_log) return 1; hal_init(); expect(os_init() == OS_STATUS_OK, "initialize OS"); expect(load_wasm(&wasm), "load yield_once.wasm"); if (wasm.bytes) { status = os_task_create(&task, wasm.bytes, wasm.size, "app_main", "yield_once", 64U * 1024U, OS_TASK_PRIORITY_NORMAL); expect(status == OS_STATUS_OK && task != NULL, "create task"); if (task) { expect(os_schedule() == OS_STATUS_OK, "run until yield"); expect(os_task_get_state(task) == OS_TASK_READY, "yield returns task to READY"); expect(os_task_get_run_count(task) == 1U, "one scheduler slice consumed"); expect(schedule_until_dead(task), "resume task after yield"); expect(os_task_get_exit_reason(task) == OS_TASK_EXIT_RETURNED, "task returned normally"); expect(os_task_get_exit_code(task) == 0U, "task returned zero"); expect(os_task_delete(task) == OS_STATUS_OK, "delete task"); } free(wasm.bytes); } expect(os_get_task_count() == 0U, "OS is clean"); os_shutdown(); hal_shutdown(); fclose(g_log); return g_failures == 0 ? 0 : 1; }
