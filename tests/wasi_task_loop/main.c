...
{
    FILE* x = fopen("wasi_task_loop.wasm", "rb");
    long n;
    if (!x)
        return 0;
    fseek(x, 0, SEEK_END);
...
int main(void)
{
    Bin b = {0};
    OsTaskHandle t = NULL;
    l = fopen("wasi_task_loop.log", "a");
    hal_init();
    ok(os_init() == OS_STATUS_OK, "initialize OS");
    ok(load(&b), "load wasi_task_loop.wasm");
    if (b.d)
    {
        ok(os_task_create(&t, b.d, b.n, "_start", "wasi_task_loop", 65536U, OS_TASK_PRIORITY_NORMAL) == OS_STATUS_OK &&
               t,
           "create WASI loop task");
        if (t)
        {
            ...
            ok(os_task_delete(t) == OS_STATUS_OK, "delete WASI loop task");
        }
        free(b.d);
    }
