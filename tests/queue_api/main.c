...
static int load(Binary* binary)
{
    ...
    if (binary->data == NULL || fread(binary->data, 1U, (size_t)size, file) != (size_t)size)
    {
        fclose(file);
        free(binary->data);
        binary->data = NULL;
...
static m3ApiRawFunction(queue_create_import)
{
    ...
    (void)_ctx;
    (void)_mem;
    if (os_queue_create(&queue, item_size, item_count) != OS_STATUS_OK)
        m3ApiReturn(0U);
...
int main(void)
{
    ...
    expect(os_host_import_register("env", "os_queue_create", "i(ii)", queue_create_import) == OS_STATUS_OK,
           "register queue create import");
    expect(os_host_import_register("env", "os_queue_delete", "i(i)", queue_delete_import) == OS_STATUS_OK,
           "register queue delete import");
    expect(load(&binary), "load queue_api.wasm");

    if (binary.data != NULL)
    {
        expect(os_task_create(&task, binary.data, binary.size, "app_main", "queue_api", 64U * 1024U,
                              OS_TASK_PRIORITY_NORMAL) == OS_STATUS_OK &&
                   task != NULL,
               "create queue API task");
        for (iteration = 0U; task != NULL && iteration < 100U && os_task_get_state(task) != OS_TASK_DEAD; ++iteration)
            expect(os_schedule() == OS_STATUS_OK, "schedule queue API task");
        if (task != NULL)
        {
