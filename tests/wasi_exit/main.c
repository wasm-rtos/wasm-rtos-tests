...
{
    FILE* x = fopen("wasi_exit.wasm", "rb");
    long n;
    if (!x)
        return 0;
    fseek(x, 0, SEEK_END);
...
int main(void)
{
    ...
    OsTaskHandle t = NULL;
    uint32_t i;
    l = fopen("wasi_exit.log", "a");
    hal_init();
    ok(os_init() == OS_STATUS_OK, "initialize OS");
    ok(load(&b), "load wasi_exit.wasm");
    if (b.d)
    {
        ok(os_task_create(&t, b.d, b.n, "_start", "wasi_exit", 65536U, OS_TASK_PRIORITY_NORMAL) == OS_STATUS_OK && t,
           "create WASI task");
        ...
        if (t)
        {
            ...
            ok(os_task_get_exit_reason(t) == OS_TASK_EXIT_EXPLICIT, "WASI task exited explicitly");
            ok(os_task_get_exit_code(t) == 7U, "WASI exit code is 7");
